...
.lgcmOptionsToMod <- function(options) {
  options <- list(
  ...
  Hed <- "# Latent Growth Curve Model\n# Generated using JASP\n"
  Int <- if (options$intercept)
    paste("I =~", paste0("1 * ", options$variables, collapse = " + "))
  else NULL
  Lin <- if (options$linear) 
    paste("L =~", paste0(options$timings, "*", options$variables, collapse = " + "))
  else NULL
  Qua <- if (options$quadratic)
    paste("Q =~", paste0(options$timings^2, "*", options$variables, collapse = " + "))
  else NULL
  Cub <- if (options$cubic)
    paste("C =~", paste0(options$timings^3, "*", options$variables, collapse = " + "))
  else NULL
  paste(Hed, Int, Lin, Qua, Cub, sep = "\n")
}
